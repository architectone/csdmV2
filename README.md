...
- **Why This Label? / What Would We Lose Without It?** — the two questions a novice actually has. The first shows why a relationship carries the label it does, by re-running the cascade under every other label CSDM would also accept — several of them validate happily and report a smaller blast radius than the truth. The second removes a node or relationship from the model on paper and reports what your model would stop being able to answer: which components lose their line to the business, which failures start under-reporting, and how much revenue-at-risk becomes unattributable. Both sit at the top of the right-click menu and the Inspector, because the point is to be found without knowing to look.
